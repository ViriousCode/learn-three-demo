// from https://www.shadertoy.com/view/w3tcWr
uniform float uTime;
...
float map(vec3 p) {
    ...
    /*
    p.y += sin(p.x + t*.5)*.2*(freqs[0] + 1.);
    p.y += sin(p.z + t*.5)*.2*(freqs[1] + 1.);
    */
    p.y += sin(p.x + t*.5)*.2;
    p.y += sin(p.z + t*.5)*.2;
...

    {
        ...
        */
        d = min(d, sdHexPrism(p, vec2((sin(h.x*100.+t)*.5+.5)*.02)));
    }
